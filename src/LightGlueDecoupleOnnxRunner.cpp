...
cv::Mat LightGlueDecoupleOnnxRunner::Extractor_PreProcess(Configuration cfg, const cv::Mat& Image, float& scale)
{
	...
	cv::Mat resizeImg = ResizeImage(tempImage, cfg.image_size, scale, fn, interp);
	cv::Mat resultImage = NormalizeImage(resizeImg);
...
int LightGlueDecoupleOnnxRunner::Extractor_Inference(Configuration cfg, const cv::Mat& image)
{
	...
	try
	{
		...
		std::cout << "[INFO] Image Size : " << image.size().height << " Channels : " << image.size().width << std::endl;
...
int LightGlueDecoupleOnnxRunner::Matcher_PostProcess(Configuration cfg, std::vector<cv::Point2f> kpts0, std::vector<cv::Point2f> kpts1)
{
	try {
		...
		std::vector<int64_t> mscore0_Shape = matcher_outputtensors[1].GetTensorTypeAndShapeInfo().GetShape();
		float* mscores0 = (float*)matcher_outputtensors[1].GetTensorMutableData<void>();//1
		printf("[RESULT INFO] mscore0 Shape : (%lld)\n", mscore0_Shape[0]);
		...
		for (int i : validIndices) {
			matches.insert(std::make_pair(matches0[i*2], matches0[i*2+1]));
		}
